...
vector<int> w = {26, 98, -91, 54, -93, -96, 65, 12, -76, -11, 78, -29, -23, 29, -34};

TEST_CASE("problem1 with our tree")
{
    Problems_1_12 pr = Problems_1_12(v);
    vector<int> deleted = pr.delete_kratni(2);
    vector<int> key = pr.get_all_keys();
    vector<int> a = v;

    CHECK((int)key.size() == 8);
...
{
    Problems_1_12 pr = Problems_1_12(w);
...
TEST_CASE("problem#3 with random tree")
{
    ...
    vector<int> key = pr.get_all_keys();
    CHECK((int)deleted.size() + (int)key.size() == 15);
    for(auto i:deleted)
        CHECK(i > comp);
    for(auto i:key)
        CHECK(i <= comp);
...
TEST_CASE("problem#5 with our tree")
{
    ...
    vector<int> key = pr.get_all_keys();
    CHECK((int)deleted.size() == 2);
    CHECK((int)key.size() == 13);
    for(int i = -7; i >= -1; i++)
        CHECK(key[i+7] == -5*i);
    for(int i = 2; i < 8; i++)
        CHECK(key[i+5] == 5*i);
...
TEST_CASE("problem#5 with random tree")
{
    ...
    vector<int> key = pr.get_all_keys();
    CHECK((int)key.size() + (int)deleted.size() == 15);
    int pos = -1;
    while(++pos < (int)key.size() && key[pos] < 0)
        CHECK(key[pos] == all[pos]);
    int i = -1;
    while(++i < (int)deleted.size() && all[i] < 10)
